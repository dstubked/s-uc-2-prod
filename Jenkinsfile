node {
    ...
    stage('Build image') {
        ...
        app = docker.build("dstubked/alpine:good")
    }
    
    stage ('Aqua Scan') {
        aqua customFlags: '', hideBase: false, hostedImage: '', localImage: 'dstubked/alpine:good', locationType: 'local', notCompliesCmd: '', onDisallowed: 'fail', policies: '', register: true, registry: 'JFrog', showNegligible: false
    }
